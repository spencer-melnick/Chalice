// Original code copyright Epic Games, Inc. All Rights Reserved.
// Modifications copyright (c) 2020 Spencer Melnick
...
UChaliceAbilityComponent* UAbilityTask_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UChaliceAbilityComponent>(AbilitySystemComponent);
}
...
void UAbilityTask_PlayMontageAndWaitForEvent::Activate()
{
	...
	UChaliceAbilityComponent* ChaliceAbilityComponent = GetTargetASC();
	...
	if (ChaliceAbilityComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = ChaliceAbilityComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAbilityTask_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (ChaliceAbilityComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UAbilityTask_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (ChaliceAbilityComponent)
	{
		ChaliceAbilityComponent->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
